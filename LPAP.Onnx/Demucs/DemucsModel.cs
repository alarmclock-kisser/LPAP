...
namespace LPAP.Onnx.Demucs
{
	public sealed class DemucsModel : IDisposable
	{
		...
		{
			this.Options = demucsOptions ?? throw new ArgumentNullException(nameof(demucsOptions));
			this.OnnxOptions = onnxOptions ?? throw new ArgumentNullException(nameof(onnxOptions));

			this.Session = CreateSessionInternal(this.Options, this.OnnxOptions);

			// Names: prefer explicit options, else take first metadata key.
			this.InputName = !string.IsNullOrWhiteSpace(this.Options.InputName)
				? this.Options.InputName!
				: this.Session.InputMetadata.Keys.First();

			this.OutputName = ResolveOutputName(this.Session, this.Options);

			// Defaults for the rest of the pipeline.
			this.ChannelsWanted = this.Options.ExpectedChannels > 0 ? this.Options.ExpectedChannels : 2;
			this.StemsWanted = this.Options.StemCount > 0 ? this.Options.StemCount : 4;

			this.FixedInputFrames = this.Options.FixedInputFrames > 0 ? this.Options.FixedInputFrames : 0;
			this.InitializeFromSessionMetadata();
		}
		...
		private void InitializeFromSessionMetadata()
		{
			...
			if (this.FixedInputFrames == 0)
			{
				try
				{
					var first = this.Session.InputMetadata.FirstOrDefault();
			...
			CancellationToken ct = default)
		{
			...
			return Task.Run(() => this.SeparateSync(interleavedInput, sampleRate, channels, progress, ct), ct);
		}
			...
			CancellationToken ct)
		{
			...
			if (channels != this.ChannelsWanted)
			{
				throw new InvalidOperationException($"DemucsModel: channels={channels} but model configured for ChannelsWanted={this.ChannelsWanted}.");
			}
			...
			float[] inputPlanar = BuildPlanarInputFixedT(
	interleavedInput.Span,
	channels,
	this.FixedInputFrames,
	enablePeakNormalize: this.Options.EnablePeakNormalize,
	out float inputPeak);

			var inputTensor = new DenseTensor<float>(inputPlanar, [1, channels, this.FixedInputFrames]);
			List<NamedOnnxValue> modelInputs = this.BuildModelInputs(inputTensor, inputPlanar, channels);

			try
			{
				using IDisposableReadOnlyCollection<DisposableNamedOnnxValue> results = this.Session.Run(modelInputs);
				...
				float[] planar = ConvertToWaveformPlanar(t, dims, this.StemsWanted, channels, this.FixedInputFrames, this.Options);
			...
			DemucsOptions options)
		{
			...
			Parallel.For(0, stemsWanted * channelsWanted, parallelOptions, (workIndex, state) =>
			{
				try
				{
					...
					int srcStem;
					var map = options?.WantedSourceIndices;
					if (map is { Length: > 0 })
					{
						int mapped = stemIndex < map.Length ? map[stemIndex] : map[^1];
						srcStem = Math.Clamp(mapped, 0, src - 1);
					}
					else
					{
						srcStem = Math.Min(stemIndex, src - 1);
					}
		...
		private static float[] BuildPlanarInputFixedT(
	ReadOnlySpan<float> interleaved, int channels, int fixedT,
	bool enablePeakNormalize,
	out float inputPeak)
		{
			...
			int srcStart = framesIn > fixedT ? (framesIn - fixedT) / 2 : 0;
			...
			int channels)
		{
			...
			{
				NamedOnnxValue.CreateFromTensor(this.InputName, waveformTensor)
			};

			foreach (KeyValuePair<string, NodeMetadata> entry in this.Session.InputMetadata)
			{
				if (string.Equals(entry.Key, this.InputName, StringComparison.Ordinal))
				{
					continue;
				}
				...
				Tensor<float> supplemental = this.CreateSupplementalInputTensor(entry.Key, entry.Value, waveformPlanar, channels);
			...
			int channels)
		{
			...
			{
				return this.BuildHybridSpectrogramInput(metadata, waveformPlanar, channels);
			}

			// Fallback: Provide a zero-initialized tensor matching the expected shape for unknown supplemental inputs.
			// Many ONNX models include optional side inputs; sending zeros is a safe default.
			...
			{
				throw new InvalidOperationException($"DemucsModel: Zusatz-Eingabe '{inputName}' vom Typ '{metadata.ElementType}' wird nicht unterstützt (nur float).");
			}

			var zero = new DenseTensor<float>(dims);
			return zero;
		}
			...
			int channels)
		{
			...
			int nFft = dims[2];
			int frames = dims[3];
			int hop = this.Options.HybridStftHopLength > 0 ? this.Options.HybridStftHopLength : (nFft / 4); // fallback


			var tensor = new DenseTensor<float>([1, 4, nFft, frames]);
			...
			for (int frame = 0; frame < frames; frame++)
			{
				...
				this.FillStftBuffer(waveformPlanar, channel: 0, frameOffset, nFft, window, buffer);
				...
				this.FillStftBuffer(waveformPlanar, channel: 1, frameOffset, nFft, window, buffer);
				Fft.ForwardInPlace(buffer);
				for (int k = 0; k < nFft; k++)
				{
		...
		private void FillStftBuffer(
			...
			Complex[] buffer)
		{
			int channelBase = channel * this.FixedInputFrames;

			for (int i = 0; i < nFft; i++)
			{
				int sampleIndex = frameOffset + i;
				float sample = (uint) sampleIndex < (uint) this.FixedInputFrames
