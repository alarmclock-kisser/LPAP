...
namespace LPAP.Audio.Processing
{
    ...
    CancellationToken ct = default)
                ...
                Parallel.For(0, ch, par, channel =>
                {
                    ...
                    PhaseVocoderHQ(
                        ...
                        progress != null && channel == 0 ? () =>
                        {
                            ...
                            if ((d & 31) == 0)
                            {
                                progress!.Report(Math.Clamp((double) d / totalProgSteps, 0, 1));
                            }
                        }
                    : null,
                        ct);
                    ...
                    if (winPV2 != null)
                    {
                        ArrayPool<float>.Shared.Return(winPV2);
                    }

                    if (winPV2Pow != null)
                    {
                        ArrayPool<float>.Shared.Return(winPV2Pow);
                    }
                });

                // Trim tail
                int lastNonZero = outData.Length - 1;
                while (lastNonZero > 0 && Math.Abs(outData[lastNonZero]) < 1e-9)
                {
                    lastNonZero--;
                }
            ...
            CancellationToken ct)
        {
            ...
            {
                return new float[Math.Max(1, inFrames)];
            }
    ...
    CancellationToken ct)
        {
            ...
            Parallel.For(0, count, new ParallelOptions { CancellationToken = ct, MaxDegreeOfParallelism = workers }, idx =>
            {
                ...
                if (refFrameStart < 0 || refFrameStart > maxStart)
                {
                    return;
                }
            ...
            CancellationToken ct)
        {
            ...
            if (isPeak != null)
            {
                ArrayPool<byte>.Shared.Return(isPeak);
            }
        }
