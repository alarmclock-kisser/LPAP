    ...
    public static partial class AudioProcessor
    {
        ...
        private sealed class CorrLocalBuffers
        {
            ...
            public CorrLocalBuffers(int ov, int ch, StereoCorrelationMode mode)
            {
                ...
                this._refA = new float[ov];
                this._candA = new float[ov];

                if (mode != StereoCorrelationMode.Mono)
                {
                    this._refB = new float[ov];
                    this._candB = new float[ov];
                }
                else
                {
            ...
            {
                Array.Copy(other._refA, this._refA, this._ov);
                this._refEA = other._refEA;

                if (this._mode != StereoCorrelationMode.Mono)
                {
                    Array.Copy(other._refB, this._refB, this._ov);
                    this._refEB = other._refEB;
                    this.SideWeight = other.SideWeight;
                }
            }

            public void FillFromOutput(float[] outInterleaved, int startFrame)
            {
                this.FillRepresentation(outInterleaved, startFrame, isSource: false);
                this.ComputeEnergyRef();
            }

            public void FillFromSource(float[] srcInterleaved, int startFrame)
            {
                this.FillRepresentation(srcInterleaved, startFrame, isSource: true);
                this.ComputeEnergyCand();
            }
            ...
            private void FillRepresentation(float[] interleaved, int startFrame, bool isSource)
            {
                ...
                bool isStereo = (this._ch == 2);

                for (int i = 0; i < this._ov; i++)
                {
                    int baseIdx = (startFrame + i) * this._ch;

                    if (!isStereo)
                    {
                        double sum = 0.0;
                        for (int c = 0; c < this._ch; c++)
                        {
                            sum += interleaved[baseIdx + c];
                        }

                        float m = (float) (sum / this._ch);
                    ...
                    switch (this._mode)
                    {
                        ...
                        case StereoCorrelationMode.Stereo:
                            {
                                if (isSource) { this._candA[i] = L; this._candB[i] = R; }
                                else { this._refA[i] = L; this._refB[i] = R; }
                                break;
                            }
                        case StereoCorrelationMode.MidSide:
                            {
                                float mid = 0.5f * (L + R);
                                float side = 0.5f * (L - R);
                                if (isSource) { this._candA[i] = mid; this._candB[i] = side; }
                                else { this._refA[i] = mid; this._refB[i] = side; }
                                break;
                            }
                    }
                }
            ...
            public double Score()
            {
                int stride = Math.Clamp(this.Stride, 1, 16);

                double dotA = Dot(this._refA, this._candA, stride);
                double denomA = Math.Sqrt(this._refEA * this._candEA) + 1e-12;
                ...
                double dotB = Dot(this._refB, this._candB, stride);
                double denomB = Math.Sqrt(this._refEB * this._candEB) + 1e-12;
                double scoreB = dotB / denomB;

                if (this._mode == StereoCorrelationMode.Stereo)
                {
                    return scoreA + scoreB;
                }

                // MidSide: weight Side a bit less (configurable)
                return scoreA + (this.SideWeight * scoreB);
            }

            private void ComputeEnergyRef()
            {
                int stride = Math.Clamp(this.Stride, 1, 16);
                this._refEA = Energy(this._refA, stride);
                this._refEB = (this._mode == StereoCorrelationMode.Mono) ? 0.0 : Energy(this._refB, stride);
            }

            private void ComputeEnergyCand()
            {
                int stride = Math.Clamp(this.Stride, 1, 16);
                this._candEA = Energy(this._candA, stride);
                this._candEB = (this._mode == StereoCorrelationMode.Mono) ? 0.0 : Energy(this._candB, stride);
            }
