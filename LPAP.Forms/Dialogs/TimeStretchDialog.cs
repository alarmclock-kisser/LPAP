...
namespace LPAP.Forms.Dialogs
{
    public partial class TimeStretchDialog : Form
    {
        ...
        private static Dictionary<NumericUpDown, int> Pow2NumericUpdowns = [];
        ...
        internal static void NumericUpDown_RegisterPow2(NumericUpDown numeric)
        {
            ...
            {
                int prev = TimeStretchDialog.Pow2NumericUpdowns.TryGetValue(numeric, out var val) ? val : 128;
        ...
        private async void button_stretch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int ti = 0; ti < trackList.Count; ti++)
                {
                    ...
                    track.TimeStretchArguments = new TimeStretchArgs(method.Name, this.StretchFactor, this.MaxWorkers, sw.Elapsed.TotalSeconds, this.ParameterValues.ToDictionary(kv => kv.Key, kv => Convert.ToDecimal(kv.Value ?? 0m)));
                    sw.Stop();
