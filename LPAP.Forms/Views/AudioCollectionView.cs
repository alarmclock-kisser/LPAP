	...
	public partial class AudioCollectionView : Form
	{
		...
		public AudioCollectionView(IEnumerable<AudioObj>? audios = null)
		{
			...
			FormClosing += (s, e) =>
			{
				...
				this.AudioC.Dispose(false);
			};

			this.Activated += (_, _) => WindowMain.LastSelectedTrackView = null;
			// Falls ein Steuerelement den Fokus erhält, ebenfalls setzen
			this.Enter += (_, _) => WindowMain.LastSelectedTrackView = null;
		...
		internal List<AudioObj> GetSelectedAudioItems()
		{
		...
		private void torchToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			var dlg = new TorchDialog(selected.First());
			dlg.ShowDialog(this);
		}
		...
		private void metricsInfoToolStripMenuItem_Click(object sender, EventArgs e)
		{
			var selected = this.GetSelectedAudioItems();
			if (selected.Count != 1)
			{
				MessageBox.Show("Please select exactly one audio track for metrics info.", "Metrics Info", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}

			MessageBox.Show(
				selected[0].GetMetricsString(),
				"Metrics Info",
				MessageBoxButtons.OK,
				MessageBoxIcon.Information);
		}

		private void timeStretchInfoToolStripMenuItem_Click(object sender, EventArgs e)
		{
			var selected = this.GetSelectedAudioItems();
			if (selected.Count != 1)
			{
				MessageBox.Show("Please select exactly one audio track for time-stretch info.", "Time-Stretch Info", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}
			if (selected[0].TimeStretchArguments == null)
			{
				MessageBox.Show("The selected audio track has no time-stretch info.", "Time-Stretch Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
				return;
			}

			MessageBox.Show(
				selected[0].TimeStretchArguments?.ToString(),
				"Time-Stretch Info",
				MessageBoxButtons.OK,
				MessageBoxIcon.Information);
		}
