	...
	public sealed class DemucsAudioObjAdapter
	{
			...
			CancellationToken ct = default)
		{
			...
			static float[] GetStem(float[][] stems, int index)
			{
				if (stems is null)
				{
					throw new InvalidOperationException("Model returned null stems.");
				}

				if ((uint) index >= (uint) stems.Length)
				{
					throw new InvalidOperationException($"Model returned only {stems.Length} stems, but index {index} was requested.");
				}
